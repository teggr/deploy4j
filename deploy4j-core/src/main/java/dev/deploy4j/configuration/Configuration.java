...
import dev.deploy4j.raw.Deploy4jYamlConfigReader;
...
public class Configuration {
  ...
  public static Configuration createFrom(ConfigureArgs configureArgs) {
    Deploy4jConfig rawConfig = Deploy4jYamlConfigReader.loadConfigFiles(
  ...
  private List<dev.deploy4j.configuration.env.Tag> envTags;
...
//    ensure_valid_service_name
  ...
  public String runDirectoryAsDockerVolume() {
    if (Paths.get(runDirectory()).isAbsolute()) {
      ...
      return Deploy4jFile.join("$(pwd)", runDirectory());
    }
  }
    ...
    if (envTags == null) {
      ...
      if (tags != null) {
        tags.entrySet().stream().forEach(entry -> {
          envTags.add(new dev.deploy4j.configuration.env.Tag(entry.getKey(), entry.getValue()));
        });
      } else {
        envTags = List.of();
      }
  ...
  public List<dev.deploy4j.configuration.env.Tag> envTag(String name) {
  ...
  public Deploy4jConfig rawConfig() {
    return rawConfig;
  }

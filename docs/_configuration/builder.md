...
If no configuration is specified, deploy4j will:
1. Create a buildx context called `deploy4j-<service>-multiarch`
...
Options go under the `builder` key in the root configuration. Example:
...
builder:
  ...
  cache:
    type: registry
    options: mode=max
    image: deploy4j-app-build-cache
...
The file also contains examples showing how to reference build args and secrets in Dockerfiles and how to expose SSH credentials to builds.
